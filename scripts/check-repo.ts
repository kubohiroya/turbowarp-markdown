...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp Markdown') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-markdown.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-markdown/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp Markdown\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/turbowarp-markdown.js')) {
